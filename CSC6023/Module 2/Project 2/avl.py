...
class AVLTree(object):
    ...
    '''
    This is the method we are adding to search through
    the tree to find the node of a given value.
    '''
    def searchTree(self, root, data):
        # if there is no root then return root
        if root is None:
            return root
        # if the node is equal to num given then return root
        elif root.data == data:
            return root
        # if the node is smaller than num given search to the right
        elif root.data < data:
