...
-- So far, we talked about how to select data from relations, how to filter the columns (using select), how to compute the Cartesian Product,
-- and how to filter the results of a Cartesian Prouct so we only get the relevent data. We also touched briefly on Rename. So let's write some review queries:
...
-- to whoever asked me for the names of all the pantry items. This is a nice way to use the rename operator to make your SQL more readable.
...
-- Or, for all the produce and dairy items:
SELECT IngredientName as PantryIngredient
FROM ingredients
...
-- The BETWEEN operator is especially useful for any attribute that could easily be tied to a range. Servings between 4 and 6. A log entry that was made between 
...
-- When we rename, we aren't limited to doing that in the select clause. We can do it in the from clause as well:
...
-- Let's pause and do some practice
