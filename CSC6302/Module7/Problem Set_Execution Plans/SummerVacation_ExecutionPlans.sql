...
-- Execution Plans Problem Set Starts here
...
FROM Activity
WHERE ActivityName NOT IN (
	SELECT DISTINCT ActivityName
    FROM SummerPlan
    WHERE ParkName IN ('Bear Brook', 'Pawtuckaway', 'Bradley Palmer')
);    

EXPLAIN ANALYZE
SELECT DISTINCT a.ActivityName
FROM Activity as a LEFT JOIN SummerPlan as sp ON a.ActivityName = sp.ActivityName
WHERE sp.ActivityName IS NULL OR sp.ParkName NOT IN ('Bear Brook', 'Pawtuckaway', 'Bradley Palmer');
...
WHERE NOT EXISTS (
	SELECT *
    FROM SummerPlan as sp
    WHERE sp.ActivityName = a.ActivityName AND sp.ParkName IN ('Bear Brook', 'Pawtuckaway', 'Bradley Palmer')
);
...
CREATE INDEX idx_state ON Park(State);
...
SELECT DISTINCT a.ActivityName
FROM Activity as a
JOIN SummerPlan as sp ON a.ActivityName = sp.ActivityName
WHERE sp.ParkName = 'YourSelectedPark' AND a.ActivityName <> 'Camping';

EXPLAIN ANALYZE
SELECT DISTINCT a.ActivityName, p.State, sp.ParkName
FROM Activity as a
JOIN SummerPlan as sp ON a.ActivityName = sp.ActivityName
JOIN Park as p ON sp.ParkName = p.ParkName
WHERE a.ActivityName IN ('Swimming', 'Kayaking', 'Paddle Boarding', 'Surfing', 'Boogeyboarding')
ORDER BY a.ActivityName, p.State, sp.ParkName;

EXPLAIN ANALYZE
SELECT DISTINCT sp.ParkName, p.State
FROM SummerPlan as sp
JOIN Park as p ON sp.ParkName = p.ParkName
WHERE sp.ActivityName IN ('Soccer', 'Baseball', 'Tennis');
...
-- 5.)
EXPLAIN ANALYZE
SELECT * FROM Park WHERE State = 'MA';

-- 6.)
EXPLAIN ANALYZE
SELECT * FROM SummerPlan WHERE ParkName = 'Bear Brook';

-- 7.)
EXPLAIN ANALYZE
SELECT * FROM SummerPlan WHERE ActivityName = 'Hiking';
